...
def Generating_Kernel(Feature_Matrix, Kernel_type, power=10, gamma= "???"):
    num_samples = Feature_Matrix.shape[0] 
    if gamma == "???":
        gamma = 1/Feature_Matrix.shape[1]
    ...
    if Kernel_type == "Gaussian_Kernel":
        ...
        for i in range(num_samples):
            for j in range(num_samples):
                ...
                # Gaussian_Kernel[i, j] = np.exp((-1 * LA.norm(Feature_Matrix[i] - Feature_Matrix[j]) ** 2) / (2 * BW ** 2))
                Gaussian_Kernel[i, j] = np.exp((-1 * gamma * LA.norm(Feature_Matrix[i] - Feature_Matrix[j]) ** 2))
        ...
        Gaussian_Kernel = np.exp(-1 * gamma * (-2 * Feature_Matrix.dot(Feature_Matrix.T) + column_vec + row_vec))
