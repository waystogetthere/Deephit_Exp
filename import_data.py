...
def import_mzz_SYNTHETIC(num_samples, num_features, norm_mode='standard'):
    ...
    label = np.asarray(df[['censored']])
    time = np.asarray(df[['survival_time']])
    data = np.asarray(df.iloc[:, :-2])
    data = f_get_Normalization(data, norm_mode)

    num_Category = int(np.max(time) * 1.2)  # to have enough time-horizon
    num_Event = int(len(np.unique(label)) - 1) + 1  # only count the number of events (do not count censoring as an event)
    ...
    return DIM, DATA, MASK
def import_1mzz_SYNTHETIC(norm_mode='standard'):
    # in_filename = './sample data/SYNTHETIC/synthetic_comprisk.csv'
    in_filename = './sample data/MZZ/2500.csv'
